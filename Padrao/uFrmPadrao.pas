...
type
  ...
  public
    ...
    Sis_Empresa, Sis_Endedeco, Sis_Bairro, Sis_UF, Sis_Fone, Sis_CEP,
    Sis_Email, Sis_Cidade, Sis_CNPJ, Sis_Fantasia, Sis_Fax : String;

    procedure PreencheVariaveisSistema;
...
procedure TFrmPadrao.PreencheVariaveisSistema;
var
  sqlRelat : TFDQuery;
  sEndereco, sBairro, sCidade, sCEP, sUF : String;
begin
  inherited;
  try
    sqlRelat := TFDQuery.Create(nil);
    sqlRelat.Connection  := SqlAuxiliar.Connection;
    sqlRelat.Transaction := SqlAuxiliar.Transaction;

    sqlRelat.Close;
    sqlRelat.SQL.Text := 'select * '+
                         '  from T008_EMPRESA';
    sqlRelat.Open;

    if not sqlRelat.IsEmpty then
    begin
      Sis_Empresa  := sqlRelat.FieldByName('T008_EMPRESA').AsString;
      Sis_Endedeco := sqlRelat.FieldByName('T008_ENDERECO').AsString;
      Sis_Bairro   := sqlRelat.FieldByName('T008_BAIRRO').AsString;
      Sis_CEP      := sqlRelat.FieldByName('T008_CEP').AsString;
      Sis_UF       := sqlRelat.FieldByName('T008_UF').AsString;
      Sis_Fone     := sqlRelat.FieldByName('T008_FONE').AsString;
      Sis_Email    := sqlRelat.FieldByName('T008_EMAIL').AsString;
      Sis_Cidade   := sqlRelat.FieldByName('T008_CIDADE').AsString;
      Sis_CNPJ     := sqlRelat.FieldByName('T008_CGC').AsString;
      Sis_Fantasia := sqlRelat.FieldByName('T008_FANTASIA').AsString;
      Sis_Fax      := sqlRelat.FieldByName('T008_FAX').AsString;
    end;
  finally
    sqlRelat.Free;
  end;
